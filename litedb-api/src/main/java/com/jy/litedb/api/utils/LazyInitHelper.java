    ...
    private void performInit() {
        if (!mHasInit) {
            synchronized (this) {
                if (!mHasInit) {
                    long ts = 0;
                    if (LiteLogUtils.INSTANCE.getSHOW_LOG()) {
                    ...
                    try {
                        ...
                        LiteLogUtils.INSTANCE.e(t);
                    }
                    if (LiteLogUtils.INSTANCE.getSHOW_LOG()) {
                        LiteLogUtils.INSTANCE.iFormat("%s init cost %s ms", mTag, SystemClock.uptimeMillis() - ts);
                    }
                }
            }
        }
